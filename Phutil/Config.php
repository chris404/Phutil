...
class Config {
	 ...
	 * 
	 * @param string $path
	 * @throws ConfigException - If a provided path is not found, or an ini file is malformed.
	 */
	public function __construct (array $paths) {
		...
		foreach($paths as $path) {
			...
			if (file_exists($path) === false) {
				throw new ConfigException(['Unkown config file path (%s)', $path]);
			}
			...
			if (is_array($iniData) === false) {
				throw new ConfigException(['Malformed config file data (%s)', $path]);
			}
	...
	public function __call($name, $args) {
		...
		if (isset($this->_data[$name]) === false) {
			throw new ConfigException(['Invalid config section (%s)', $name]);
		}
		...
		foreach($args as $configKey) {
			...
			if(isset($data[$configKey]) === false) {
				throw new ConfigException(['Invalid config key (%s) (%s)', $name, $configKey]);
			}
